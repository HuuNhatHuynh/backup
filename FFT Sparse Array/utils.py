...
class ArrayModelAbstractClass(ABC):
    ...
    def plot_coarray_weight(self, plot3D=False) -> None:
        ...
        if plot3D:
            ...
            points = go.Scatter3d(x=posx, y=posy, z=posz,
                                mode='markers',
                                marker=dict(size=4, color=posz, colorscale='viridis'),
                                name='co-array weight')
            
            lines = []
            for xi, yi, zi in zip(posx, posy, posz):
            ...
            fig = go.Figure(data=go.Scatter(x=posx, y=posy,
                            mode='markers',
                            marker=dict(
                                size=6,
                                color=posz,
    ...
    def estimate_doa_fft(self, X, nbSources, fft_vertical_size=256, fft_horizontal_size=256, show_fft=False):
        ...
        for j, i in top_peaks:
            ...
            b1 = self.lamda / self.min_distance * ((i >= fft_horizontal_size + 1) - (i / (2 * fft_horizontal_size + 1)))
            b2 = self.lamda / self.min_distance * ((j >= fft_vertical_size + 1) - (j / (2 * fft_vertical_size + 1)))
...
def get_displacement(y, w2=1/4, w1=1/2, w0=1):
    ...
    w = np.array([w2, w1, w2, w1, w0, w1, w2, w1, w2])
    A = A * np.sqrt(w).reshape(-1, 1)
    y = y.flatten() * np.sqrt(w)

    a, b, c, d, e, _ = np.linalg.lstsq(A, y)[0] 

    if (4 * a * b > c * c and a > 0) or (4 * a * b <= c * c):

        return np.array([0, 0])
    
    return  np.linalg.solve(np.array([[2 * a, c], [c, 2 * b]]), np.array([- d, - e]))
