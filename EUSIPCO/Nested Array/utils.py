...
def data_augmentation(observations: torch.Tensor, angles: torch.Tensor, nbCopies: int=10):

    n, t, _ = observations.shape
    observations = observations.repeat_interleave(repeats=nbCopies, dim=0)
    random_transformations = torch.exp(1j * 2 * torch.pi * torch.rand(n * nbCopies, t, 1))
    aug_observations = observations * random_transformations
    aug_angles = angles.repeat_interleave(repeats=nbCopies, dim=0)

    return aug_observations, aug_angles
