...
# class DoAConvLSTM(nn.Module):

#     def __init__(self, m: int, d: int, tau: int=8, device='cuda'):
        
#         super().__init__()
#         self.m = m
#         self.d = d
#         self.tau = tau
#         self.convlstm = ConvLSTM(input_dim=2, hidden_dim=1, kernel_size=(3, 3), num_layers=1, batch_first=True, bias=True, return_all_layers=False, device=device)
#         self.mlp = nn.Sequential(nn.Linear(in_features=self.m*self.m, out_features=self.m*self.m, device=device), nn.ReLU(),
#                                  nn.Linear(in_features=self.m*self.m, out_features=self.m*self.m, device=device), nn.ReLU(),
#                                  nn.Linear(in_features=self.m*self.m, out_features=self.d, device=device))
        
#     def forward(self, x: torch.Tensor):

#         b, t, _ = x.shape

#         Rx = torch.zeros((b, self.tau, self.m, self.m), dtype=torch.complex64, device=x.device)
#         for i in range(self.tau):
#             Rx[:, i, :, :] = x[:, i:, :].transpose(1, 2) @ x[:, :t-i, :].conj() / (t-i)
#         Rx = torch.stack((torch.real(Rx), torch.imag(Rx)), dim=2)
#         _, last_states = self.convlstm(Rx)
#         y = torch.flatten(last_states[0][0], start_dim=1, end_dim=-1)
#         y = self.mlp(y)
#         return y
...
# def adversarial_train(model, nbEpoches, lr, wd, train_loader, valid_loader, model_name, train_func, validation_func, device=dev):
...
#             loss = train_func(model(X), theta_true) 
#             loss.backward()
#             X_perturbed = X + epsilon * torch.sgn(X.grad)
            
#             optimizer.zero_grad()
#             theta_pred = model(X)
#             theta_pred_perturbed = model(X_perturbed)
#             loss_ad = alpha * train_func(theta_pred, theta_true) + (1 - alpha) * train_func(theta_pred_perturbed, theta_true)
#             loss_ad.backward()
#             optimizer.step()
#             running_loss += loss.item()
