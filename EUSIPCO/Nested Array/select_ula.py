...
observations_test, angles_test = generate_data(n, t, d, snr, snr, array, False, False)
...
for i in range(nbTrain):
    ...
    da_music.load_state_dict(torch.load(path+'da_music_'+str(snr)+'dB_ula_'+str(i)+'.pth', weights_only=True))
    running_loss = 0.0
    with torch.no_grad():
    ...
    if lossavg < MIN_DAMUSIC:
        MIN_DAMUSIC = lossavg
        torch.save(da_music.state_dict(), path+'da_music_'+str(snr)+'dB_ula.pth')
...
for i in range(nbTrain):
    ...
    da_music_v2.load_state_dict(torch.load(path+'da_music_v2_'+str(snr)+'dB_ula_'+str(i)+'.pth', weights_only=True))
    running_loss = 0.0
    with torch.no_grad():
        for data in test_loader:
            X, theta_true = data[0].to(dev), data[1].to(dev)
    ...
    if lossavg < MIN_DAMUSICv2:
        ...
        torch.save(da_music_v2.state_dict(), path+'da_music_v2_'+str(snr)+'dB_ula.pth')
...
for i in range(nbTrain):
    ...
    rnn = RNN(m, d, dev)    
    rnn.load_state_dict(torch.load(path+'rnn_'+str(snr)+'dB_ula_'+str(i)+'.pth', weights_only=True))
    running_loss = 0.0
    with torch.no_grad():
        for data in test_loader:
            X, theta_true = data[0].to(dev), data[1].to(dev)
    ...
    if lossavg < MIN_RNN:
        MIN_RNN = lossavg
        torch.save(rnn.state_dict(), path+'rnn_'+str(snr)+'dB_ula.pth')