...
for i in range(nbTrain):
    ...
    da_music.load_state_dict(torch.load(path+'da_music_'+str(snr)+'dB_nested_'+str(i)+'.pth', weights_only=True))
    running_loss = 0.0
    with torch.no_grad():
...
for i in range(nbTrain):
    ...
    da_music_v2.load_state_dict(torch.load(path+'da_music_v2_'+str(snr)+'dB_nested_'+str(i)+'.pth', weights_only=True))
    running_loss = 0.0
    with torch.no_grad():
        for data in test_loader:
            X, theta_true = data[0].to(dev), data[1].to(dev)
...
for i in range(nbTrain):
    ...
    rnn = RNN(m, d, dev)    
    rnn.load_state_dict(torch.load(path+'rnn_'+str(snr)+'dB_nested_'+str(i)+'.pth', weights_only=True))
    running_loss = 0.0
    with torch.no_grad():
        for data in test_loader:
            X, theta_true = data[0].to(dev), data[1].to(dev)
