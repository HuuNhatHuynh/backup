...
class DA_RARE(nn.Module):

    def __init__(self, m: int, d: int, mc_range: int, array, device=dev):
        ...
        self.transform_matrices = array.transform_matrices.to(device)
    ...
    def forward(self, x):
        ...
        Y = torch.einsum('smc, bmk->bsck', W.conj(), En)
        Q = Y @ Y.conj().transpose(-2, -1)
