...
def generate_two_direction(gap: float, angle_min: float, angle_max: float):

    theta1 = torch.rand(1) * (angle_max - gap - angle_min) + angle_min 
    theta2 = theta1 + gap
...
def generate_signal(t: int, d: int, snr: float, array, qpsk: bool, coherent: bool, gap: float, angle_min: float, angle_max: float):
    ...
    theta = generate_two_direction(gap, angle_min, angle_max)
...
class Root_MUSIC:
    ...
    def estimate(self, X):
        ...
        theta = - torch.arcsin(torch.angle(roots) * self.lamda / (2 * torch.pi * self.distance))
