...
def generate_signal(t: int, d: int, snr: float, array, coherent: bool, gap: float, angle_min: float, angle_max: float):
    """
    ...
    if not coherent:
        ...
        x = ((0.8 + 0.4 * torch.rand(d, 1)) * torch.exp(1j * 2 * torch.pi * torch.rand(d, 1))) @ x
        # x = x.repeat(d, 1)
...
def generate_data(n: int, t: int, d: int, snr_min: float, snr_max: float, array, coherent: bool, 
    ...
    for _ in range(n):
        ...
        y, theta = generate_signal(t, d, snr, array, coherent, gap, angle_min, angle_max)
...
class MUSIC:
    ...
    def estimate(self, X: torch.Tensor, spatial_smoothing: bool = False):

        cov = X @ X.T.conj() / X.shape[1]

        if spatial_smoothing:
            assert cov.shape[0] > self.m, "Dimension of covariance must be greater than number of sensors"
            sum_cov = torch.zeros(self.m, self.m, dtype=torch.complex64)
            for i in range(cov.shape[0] - self.m + 1):
                sum_cov += cov[i:i+self.m, i:i+self.m]
            cov = sum_cov / (cov.shape[0] - self.m + 1)
