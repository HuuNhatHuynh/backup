...
class TruncatedGaussianDistribution:
    ...
    def __init__(self, val_min: float, val_max: float, loc: float, scale: float):

        self.val_min = val_min
        self.val_max = val_max
        self.loc = loc
        self.scale = scale
        self.factor = norm.cdf(val_max, loc=loc, scale=scale)-norm.cdf(val_min, loc=loc, scale=scale)
        self.pdf = lambda x: np.exp(-((x-loc)**2)/2)/(self.factor*scale*np.sqrt(2*np.pi))

    def sample(self, size: int, total: int = 10000):

        x = np.random.rand(total) * (self.val_max-self.val_min) + self.val_min
        y = np.random.rand(total) / (self.factor*self.scale*np.sqrt(2*np.pi))
        idx = y < self.pdf(x)
        x = x[idx]
        return x[:size]