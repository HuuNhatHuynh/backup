...
class RNN(nn.Module):
    ...
    def __init__(self, m: int, dmin: int, dmax: int):
        ...
        self.dmin = dmin
        self.dmax = dmax
        self.rnn = nn.GRU(2*m, 2*m, num_layers=2, batch_first=True)
        self.mlp = nn.Sequential(nn.Linear(2*m, 2*m), nn.ReLU(), 
                                nn.Linear(2*m, 2*m), nn.ReLU(), 
                                nn.Linear(2*m, dmax-dmin+1), nn.Softmax(dim=1))
        
    def forward(self, x: torch.Tensor):

        x = torch.cat((torch.real(x), torch.imag(x)), dim=2)
        _, x = self.rnn(x)
        y = self.mlp(x)
